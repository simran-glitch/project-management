...
public class UploadToDBServlet extends HttpServlet {
        ...
        try {
            ...
            for (Part part : request.getParts()) {
                ...
                if (fileName != null && fileName.length() > 0) {
                    ...
                    // Write to file
                    this.writeToDB(conn, studentName, fileName, is, description);
                }
            }
            conn.commit();
    ...
    private Long getMaxprojectstId(Connection conn) throws SQLException {
        String sql = "Select max(a.id) from Projects a";
    ...
    private void writeToDB(Connection conn,String studentName, String fileName, InputStream is, String description) throws SQLException {
 
        String sql = "Insert into PROJECTS(Id,Student_Name,File_Name,File_Data,Description) " //
                + " values (?,?,?,?,?) ";
        PreparedStatement pstm = conn.prepareStatement(sql);
 
        Long id = this.getMaxprojectstId(conn) + 1;
        pstm.setLong(1, id);
